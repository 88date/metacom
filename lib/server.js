...
class Client extends EventEmitter {
  ...
  centrifugoError(code, message, options) {
    this.#transport.centrifugoError(code, message, options);
  }

  centrifugoDomainError(code, message, options) {
    this.#transport.centrifugoDomainError(code, message, options);
  }
  ...
  hook(obj, code, ext) {
    this.#transport.hook(obj, code, ext);
  }
  ...
  emit(name, data, id) {
    ...
    this.sendEvent(name, data, id);
  }

  sendEvent(name, data, id) {
    const packet = { id, type: 'event', name, data };
  ...
  async loadSession() {
    ...
    const token = this.#transport.getToken();
    if (!token) return;
    const restored = this.restoreSession(token);
    if (restored) return;
    const session =
      await this.#transport.server.application.auth.readSession(token);
    if (session) this.startSession(token, session);
  }
...
class Server {
  ...
  getRawRouter(url) {
    ...
    const path = metautil.split(pathname, '?')[0];
    const unit = metautil.split(path, '/')[0];
    const hook = this.application.getHook(unit);
    const router = hook?.router;
    if (router?.exports?.raw !== true) return null;
    return router;
  }

  async raw(proc, req, res) {
    const { method } = proc;
  ...
  init() {
    ...
    const { protocol, nagle = true, key, cert, SNICallback, secret } = options;
    ...
    this.httpServer.on('request', async (req, res) => {
      ...
      if (balancer) {
        const transport = new HttpTransport(this, req, res);
        if (!res.writableEnded) this.balancing(transport);
        return;
      }
      ...
      if (req.url.startsWith('/api/centrifugo/')) {
        const transport = new HttpTransport(this, req, res);
        if (res.writableEnded) return;
        const client = new Client(transport);
        const data = await metautil.receiveBody(req).catch(() => null);
        const proxySecret = Buffer.from(req.headers.secret || '');
        const validLength = expectedSecret.length === proxySecret.length;
        const validSecret =
          validLength && timingSafeEqual(expectedSecret, proxySecret);
        if (req.method !== 'POST' || !validSecret) {
          transport.centrifugoError(403, 'unauthorized');
        } else {
          this.centrifugo(client, req.url, data);
        }
        return;
      }

      const rawRouter = this.getRawRouter(req.url);
      if (rawRouter) {
        const options = { handleOptions: false };
        const transport = new HttpTransport(this, req, res, options);
        await this.raw(rawRouter, transport.req, transport.res);
        return;
      }
  ...
  async rpc(client, packet) {
    ...
    const [unitName, methodName] = method.split('/');
    const [unit, ver = '*'] = unitName.split('.');
    const proc = this.application.getMethod(
      unit,
      ver,
      methodName,
      client.transport,
    ...
    try {
      await proc.enter(context.client.ip);
    } catch {
    ...
    try {
      ...
      return void client.error(code, { id, error, url: method });
    } finally {
      proc.leave(context.client.ip);
    }
    if (metautil.isError(result)) {
      const { code, httpCode = 200 } = result;
      return void client.error(code, {
        id,
        error: result,
        httpCode,
        url: method,
  ...
  request(client, transport, data) {
    ...
    if (hook) {
      const { router, ext } = hook;
      this.hook(client, router, packet, verb, headers, ext);
    } else {
      this.rpc(client, packet);
    }
  }

  async hook(client, proc, packet, verb, headers, ext) {
    const { id, method, args } = packet;
    if (!proc) return void client.error(404, { id });
    const context = client.createContext();
    try {
    ...
    if (ext) {
      client.hook(result, 200, ext);
    } else {
      client.send(result, 200);
    }
    this.console.log(`${client.ip}\t${method}`);
  }
  ...
  async centrifugo(client, url, jsonData) {
    const route = url.slice('/api/centrifugo/'.length);
    const payload = metautil.jsonParse(jsonData) || {};

    if (route === 'connect') {
      if (payload.data && payload.data.token) {
        const { token, language, appVersion } = payload.data;

        const session = await this.application.auth.readSession(token);
        if (!session) {
          const packet = { disconnect: { code: 4501, reason: 'unauthorized' } };
          return void client.send(packet);
        }

        const sessionState = { ...session };
        if (language) sessionState.language = language;
        if (appVersion) sessionState.appVersion = appVersion;
        client.startSession(token, sessionState);

        const { state } = client.session;
        const meta = { ...state, token };
        client.send({ result: { user: meta.userId, meta } });
        this.console.log(`${client.ip}\tcentrifugo\tconnect`);
        return;
      } else {
        client.send({ result: { user: '' } });
        this.console.log(`${client.ip}\tcentrifugo\tconnect\tanonymous`);
        return;
      }
    }
    ...
    if (meta && meta.token) {
      client.startSession(meta.token, meta);
    }

    if (route === 'rpc') {
      const { method, data: rpcData } = args;
      if (!method) {
        return void client.centrifugoError(400, 'RPC method required');
      }

      const { methodName, unit, ver } = parseRoute(method);
      await this.centrifugoCall(client, unit, ver, methodName, rpcData, method);
    } else if (route === 'subscribe' || route === 'publish') {
      const label = `channel.1/${route}\t${args.channel}`;
      await this.centrifugoCall(client, 'channel', '1', route, args, label);
    } else {
      client.centrifugoError(404, `Unknown centrifugo proxy route: ${route}`);
    }
  }

  async centrifugoCall(client, unit, ver, methodName, args, label) {
    const opts = { method: label };
    const proc = this.application.getMethod(
      unit,
      ver,
      methodName,
    ...
    if (!proc) {
      return void client.centrifugoDomainError(404);
    }
    const context = client.createContext();
    if (!client.session && proc.access !== 'public') {
      return void client.centrifugoError(403, 'Forbidden', opts);
    }
    try {
      await proc.enter(context.client.ip);
    } catch {
      return void client.centrifugoDomainError(503);
    }
    let result = null;
    try {
      result = await proc.invoke(context, args);
      ...
      if (typeof error.code === 'number') code = error.code;
      error.httpCode = code <= 599 ? code : 500;
      return void client.centrifugoDomainError(code, { error, url: label });
    } finally {
      proc.leave(context.client.ip);
    }
    if (metautil.isError(result)) {
      const { code, httpCode = 200 } = result;
      return void client.centrifugoDomainError(code, {
        error: result,
        httpCode,
        url: label,
      });
    }
    client.send({ result: { data: { ok: true, result } } });
    this.console.log(`${client.ip}\t${label}`);
  }
}
