  ...
  async centrifugo(client, url, jsonData) {
    ...
    if (route === 'connect') {
      if (payload.data && payload.data.token) {
        const { token, ...state } = payload.data;

        let session = await this.application.auth.readSession(token);
        session = { ...session, ...state };
        if (!session) {
          const packet = { disconnect: { code: 4501, reason: 'unauthorized' } };
          return void client.send(packet);
        }
        client.startSession(token, session);

        const { state: meta } = client.session;
