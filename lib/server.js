...
class Server {
  ...
  message(client, data) {
    ...
    else if (type === 'ack' && id) return void this.ack(client, packet);
  ...
  async ack(client, packet) {
    const { id, name } = packet;
    await this.application.ack.complete(id);
    this.console.log(`${client.ip}\tack ${name}`);
  }
  ...
  async centrifugo(client, url, data) {
    const type = url.slice('/api/centrifugo/'.length);
    ...
    } else if (type === 'subscribe' || type === 'publish') {
      const label = `channel.1/${type}\t${args.channel}`;
      await this.centrifugoCall(client, 'channel', '1', type, args, label);
    } else {
      client.centrifugoError(404, `Unknown centrifugo proxy type: ${type}`);
    }
  }
  ...
  async centrifugoCall(client, unit, ver, methodName, args, label) {
    ...
    const proc = this.application.getMethod(unit, ver, methodName);
    if (!proc) {
      return void client.centrifugoError(
        404,
        `Method not found: ${label}`,
        opts,
      );
    }
    const context = client.createContext();
    ...
    try {
      ...
      return void client.centrifugoError(503, 'Service unavailable', opts);
    }
    let result = null;
    try {
      result = await proc.invoke(context, args);
      ...
      return void client.centrifugoError(code, error.message, {
        ...opts,
        error,
      });
    } finally {
    ...
    if (metautil.isError(result)) {
      ...
      const numCode = typeof code === 'number' ? code : 400;
      const msg = typeof code === 'string' ? `${code}: ${message}` : message;
      return void client.centrifugoError(numCode, msg, {
        ...opts,
        error: result,
      });
    }
    client.send({ result: { data: result } });
    this.console.log(`${client.ip}\tcentrifugo\t${label}`);
  }
}
