...
class Transport extends EventEmitter {
  ...
  error(code = 500, { id, error = null, httpCode = null, url } = {}) {
    const { server, req, ip } = this;
    const { console } = server;
    const method = url ? 'POST' : req.method;
    ...
    console.error(`${ip}\t${method}\t${url || req.url}\t${reason}`);
  ...
  log(code) {
    const { server, req, ip } = this;
    if (req.url.startsWith('/api/centrifugo')) return;
    const { console } = server;
  ...
  centrifugoDomainError(
    code = 500,
    { error = null, httpCode = null, url } = {},
  ) {
    const { server, req, ip } = this;
    const { console } = server;
    const method = url ? 'POST' : req.method;
    ...
    const message = pass && error ? error.message : status || 'Unknown error';
    const reason = `${httpCode}\t${code}\t${error ? error.stack : status}`;
    console.error(`${ip}\tcentrifugo\t${method}\t${url || req.url}\t${reason}`);
    const outCode = pass ? code : httpCode;
    const outError = { message, code: outCode };
    const packet = { result: { data: { ok: false, error: outError } } };
    this.send(packet);
  }

  centrifugoError(code, message, { method, error } = {}) {
    const { server, req, ip } = this;
    const { console } = server;
    const url = method || req.url;
    const stack = error ? error.stack : message;
    console.error(`${ip}\tcentrifugo\t${url}\t${code}\t${stack}`);
    const packet = { error: { code, message } };
    this.send(packet);
  }
  ...
  hook(obj, code = 200, ext = 'json') {
    const data = ext === 'json' ? JSON.stringify(obj) : obj;
    this.write(data, code, ext);
  }

  getToken() {
    const { token } = this.req.headers;
    return token || null;
  }
}
...
class HttpTransport extends Transport {
  constructor(server, req, res, options = {}) {
    super(server, req, 'http');
    this.res = res;
    const { handleOptions = true } = options;
    if (handleOptions && req.method === 'OPTIONS') this.options();
    req.on('close', () => {
      this.emit('close');
    });
  }
  ...
  close() {
    if (this.req.url.startsWith('/api/centrifugo/')) {
      this.send({ disconnect: { code: 4500, reason: 'closed' } });
      return;
    }
    this.error(503);
